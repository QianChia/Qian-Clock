...
let app = new Vue({
    ...
    methods: {
        ...
        getTime() {
            if (localStorage.getItem("Clock-isShowChineseDate") == 1) {
                this.date = getLunarDate(new Date());
            } else {
                this.date = new Date();
            }
        },
    ...
    computed: {
        ...
        year() {
            if (localStorage.getItem("Clock-isShowChineseDate") == 1) {
                return formatDateY_lunar(new Date());
            } else {
        ...
        monthList() {
            ...
            for (let i = 0; i < total; i++) {
                let monthStr = this.cn(i + 1) + '月';
                if (localStorage.getItem("Clock-isShowChineseDate") == 1) {
                    if (monthStr == '一月') {
                        monthStr = '正月';
                    } else if (monthStr == '十一月') {
                        monthStr = '冬月';
                    } else if (monthStr == '十二月') {
                        monthStr = '腊月';
                    } 
                }
                list.push({
        ...
        dayList() {
            ...
            for (let i = 1; i <= total; i++) {
                let dayStr = this.cn(i) + '日';
                if (localStorage.getItem("Clock-isShowChineseDate") == 1) {
                    if (i < 11) {
                        dayStr = '初' + dayStr.replace('日', '');
                    }
                }
                list.push({
                    label: dayStr,
...
function formatDateY_lunar(value) {
    var date_Y = formatDateY(new Date(value));	
    var date_M = formatDateM(new Date(value));
    var date_D = formatDateD(new Date(value));
    var lunarDate = solar2lunar(date_Y, date_M, date_D);
